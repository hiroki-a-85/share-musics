...
class UploadSizeCheckMiddleware
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        $post_max_size = mb_substr(ini_get('post_max_size'),0,2) * 1024 * 1024;
        
        //PHP:リクエストデータ量をバイトで取得する - $request->server('CONTENT_LENGTH')
        
        $uploaded_size = $request->server('CONTENT_LENGTH');
